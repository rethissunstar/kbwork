...
import keycapData from "../seeds/keycapsData.json" assert { type: "json" };
...
async function uploadImages() {
  ...
  const imageDir = path.join(currentDirectory, "../assets/images/keycaps");
  ...
  for (const image of images) {
    ...
    const product = keycapData.find((p) => p.imageRef === imageRef);
    if (product) {
      product.imageURL = s3UploadResponse.Location;
    }
  }

  const keycapDataPath = path.join(currentDirectory, "../seeds/keycapsData.json");
  await fs.writeFile(keycapDataPath, JSON.stringify(keycapData, null, 2));
}
